...
async fn test_cognito_auth_no_groups() {
    ...
    let response_value = serde_json::to_value(response).unwrap();
    assert_eq!(response_value["errorType"], "Unauthorized");
}
...
async fn test_cognito_auth_with_admin_group() {
    ...
    let response_value = serde_json::to_value(response).unwrap();
    assert_eq!(response_value["data"]["id"], player.id.to_string());
}
...
async fn test_iam_auth() {
    ...
    let response_value = serde_json::to_value(response).unwrap();
    assert!(response_value.get("data").is_some_and(|v| v.is_array()));
}
...
async fn test_get_inexistent_player() {
    ...
    let response_value = serde_json::to_value(response).unwrap();
    assert!(response_value.get("data").is_none() || response_value["data"].is_null());
}
...
async fn test_create_multiple_players() {
    ...
    for name in &names {
        ...
        let create_lambda_event =
            lambda_runtime::LambdaEvent::new(create_event, Default::default());
...
async fn test_delete_player_twice() {
    ...
    let delete_event: serde_json::Value = json!({
        "info": {
            "fieldName": "deletePlayer",
            "parentTypeName": "Mutation",
            "variables": {},
